...
header('location:login.php');
	...
	$idm1= $hasil+1;
	$idm  = (string)$idm1;
	for($i=0;$i<strlen($idm);++$i){
        $idpenc.=gmp_strval(gmp_mod(gmp_pow(ord($idm[$i]),$e),$n));
        if($i!=strlen($idm)-1){
            $idpenc.=".";
        }
    }
		foreach($value as $qty=> $val34){
mysqli_query($con,"insert into orders(id,userId,productId,quantity) values('$idpenc','".$id."','$qty','$val34')");
